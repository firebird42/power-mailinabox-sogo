...
user_query = SELECT email AS user, "mail" as uid, "mail" as gid, "$STORAGE_ROOT/mail/mailboxes/%d/%n" as home, '*:bytes=' || quota AS quota_rule FROM miab_users WHERE email='%u';
...
hosts = 127.0.0.1
dbname = mailinabox
query = SELECT 'REJECT This address is not ready to receive email.' FROM miab_noreply WHERE email='%s';
EOF
